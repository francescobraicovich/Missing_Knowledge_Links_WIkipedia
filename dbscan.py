...
def dbscan_from_similarity(similarity_matrix, subcluster_noise=True, verbose=True):
    ...
    if noise_size > 0.1 * n and subcluster_noise and noise_size > 50:
        print('')
        print(f'Warning: The noise cluster is too large, Noise size: {noise_size}, Threshold: {int(0.1 * n)}')
        ...
        noise_labels = dbscan_from_similarity(noise_similarity_matrix, subcluster_noise=True, verbose=False)
